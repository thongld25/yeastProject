...
import { getExperimentByUserId } from "../../services/ExperimentService";

const ImageOfEmployee = () => {
  useUserAuth();
  ...
  const fetchImagesOfUser = async (page = 1) => {
    try {
      let res;
      if(searchKeyword.trim() || experimentId || measurementId) {
        res = await searchImagesOfUser(page, limit, {
          name: searchKeyword,
          experimentId,
          measurementId,
        });
      } else {
        res = await getImagesByUserIdPage(page, limit);
      }
      console.log("res", res);
      if (res.status === 200) {
  ...
  useEffect(() => {
    const fetchDropdownData = async () => {
      try {
        const expRes = await getExperimentByUserId(); // hoặc getExperimentsByUser(user._id)
...
export default ImageOfEmployee;
